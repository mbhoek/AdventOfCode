...
namespace AdventOfCode2023
{
	static class Day02
	{
		internal static void Solve()
		{
			...
			var games = @"Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
Game 100: 6 green, 15 red, 12 blue; 9 red; 16 red; 17 red, 3 blue, 7 green";
			...
			foreach (var game in games.Split(Environment.NewLine))
			{
				...
				Debug.Assert(gameParts.Length == 2);
				var info = gameParts[0];
				var grabs = gameParts[1];

				var infoParts = info.Split(' ');
				Debug.Assert(infoParts.Length == 2);
				var id = Convert.ToInt32(infoParts[1]);

				int red = 0, green = 0, blue = 0, maxCubeCount = 0;
				foreach (var grab in grabs.Split(';', StringSplitOptions.TrimEntries))
				{
					var cubeCount = 0;
					foreach (var cube in grab.Split(',', StringSplitOptions.TrimEntries))
					{
						var cubeParts = cube.Split(' ');
						Debug.Assert(cubeParts.Length == 2);
						var count = Convert.ToInt32(cubeParts[0]);
